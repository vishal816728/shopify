...
import teacherModel from "../../Models/Registration/teacher.model.js";
...
class ForgotPassword extends Controller {
    ...
    async forgotPasswordVerficationOtp(req, res) {
        ...
        try {
            ...
            const isStudent = await userModel.findOne({ email })
            if (isStudent) {
    ...
    async updatePassword(req, res) {
        ...
        try {
            const isStudent = await userModel.findById(id)

            if (isStudent) {
                const updatePassword = await isStudent.updateOne(
                    {
                ...
                const updatePassword = await teacherModel.findByIdAndUpdate({
                    _id: id
                }, {
                    password: encryptedPass
                })
